...
func (td *TwinData) GetPayload() ([]byte, error) {
	...
	if strings.Contains(td.Topic, "$hw") {
		...
		// klog.Infof("enter create message twin update %v", payload)
	} else {
		...
		klog.Infof("enter create message data", payload)
	}

	klog.Infof("Get the %s value as %s", td.Name, sData)
	return payload, nil
}
