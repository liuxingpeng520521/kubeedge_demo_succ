...
func initTwin(ctx context.Context, dev *CustomDev) {
...
// start the device.
func (d *DevPanel) start(ctx context.Context, dev *CustomDev) {
...
func NewDevPanel() *DevPanel {
	if devPanel == nil {
		devPanel = &DevPanel{
			...
			devices:    make(map[string]*CustomDev),
...
func (d *DevPanel) UpdateDev(model *common.DeviceModel, device *common.DeviceInstance, protocol *common.Protocol) {
	d.devices[device.ID] = new(CustomDev)
